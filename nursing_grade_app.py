...
# API 설정 영역
with st.expander("🔑 AI API 설정 (Gemini / Anthropic)", expanded=False):
    ai_provider = st.radio("사용할 AI 모델 선택", ["Gemini (추천)", "Anthropic (Claude)"], horizontal=True)
    if ai_provider == "Gemini (추천)":
        gemini_api_key = st.text_input("Gemini API Key 입력", type="password", help="Google AI Studio에서 발급받은 API 키를 입력하세요.")
        st.markdown("[Gemini API 키 발급받기](https://aistudio.google.com/app/apikey)")
    else:
        anthropic_api_key = st.text_input("Anthropic API Key 입력", type="password", help="Anthropic Console에서 발급받은 API 키를 입력하세요.")
...
if st.button("🤖 AI 컨설팅 보고서 생성", type="primary", use_container_width=True):
    if ai_provider == "Gemini (추천)" and not gemini_api_key:
        ...
        st.error("❌ Anthropic API 키를 입력해 주세요.")
    else:
